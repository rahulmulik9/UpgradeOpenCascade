...
std::string trim_gr(const std::string& str,
...
std::string gtrim(const std::string& str,
    const std::string& whitespace = " \t")
{
    const auto strBegin = str.find_first_not_of(whitespace);
    if (strBegin == std::string::npos)
...
VrmlData_ErrorStatus VrmlData_Group::Read(VrmlData_InBuffer& theBuffer)
{
    ...
    {
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "whichChoice")) {
            VrmlData_Scene::readLine(theBuffer);
        }
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "-1")) {
            VrmlData_Scene::readLine(theBuffer);
            continue;
        }
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "ROUTE") || VRMLDATA_LCOMPARE(theBuffer.LinePtr, "EXTERNPROTO")) {
            VrmlData_Scene::readLine(theBuffer);
            continue;
        }
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "description") || VRMLDATA_LCOMPARE(theBuffer.LinePtr, "parameter")) {
            VrmlData_Scene::readLine(theBuffer);
            continue;
        }
        ...
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "bboxCenter"))
        ...
        else if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "choice")) {
            int i = 0;
            ...
            while (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
                ...
                if (std::count(VrmlData_Proto::protos.begin(), VrmlData_Proto::protos.end(), str.c_str())) {
                    const_cast<VrmlData_Scene*>(myScene)->createNode(theBuffer, aChildNode, NULL);
                }
                else if (!OK(aStatus, ReadNode(theBuffer, aChildNode)))
                    break;
                AddNode(aChildNode);
                std::string brack = theBuffer.Line;
                if (trim_gr(brack) == "]") {
                    //theBuffer.LinePtr++;
